...
function ensureSliderStyles(): void {
	if (typeof document === "undefined" || document.getElementById(SLIDER_CSS_ID))
		return;
	const primary = lightColorScheme.primary;
	const outline = lightColorScheme.outline;
	const surface = lightColorScheme.surface;
	const focusRing = "0 0 0 2px rgba(22, 119, 255, 0.2)";
	const duration = motionTokens.duration.short2;
...
.surface-slider::-webkit-slider-runnable-track{height:6px;border-radius:999px;background:${outline}}
.surface-slider::-webkit-slider-thumb{appearance:none;-webkit-appearance:none;width:20px;height:20px;border-radius:50%;background:${surface};border:2px solid ${primary};box-shadow:0 1px 2px rgba(0,0,0,0.1);margin-top:-7px;transition:transform ${duration}, box-shadow ${duration}}
.surface-slider::-webkit-slider-thumb:hover{transform:scale(1.05)}
.surface-slider:focus{outline:none}
.surface-slider:focus::-webkit-slider-thumb{box-shadow:${focusRing}}
...
.surface-slider:disabled{opacity:${disabledOpacity};cursor:not-allowed}
.surface-slider::-moz-range-track{height:6px;border-radius:999px;background:${outline}}
.surface-slider::-moz-range-thumb{width:20px;height:20px;border-radius:50%;background:${surface};border:2px solid ${primary};cursor:pointer;transition:transform ${duration}, box-shadow ${duration}}
...
.surface-slider:focus::-moz-range-thumb{box-shadow:${focusRing}}
